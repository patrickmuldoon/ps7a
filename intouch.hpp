...
const std::string OriginalVersion = "^([A-Z?][a-z]{2})\\s(\\d{2})\\s(\\d{2}):(\\d{2}):(\\d{2}).*intouch-platform-base-(\\d)\\.(\\d)\\.(\\d)\\-(\\w+\\.\\d{2}|\\d{1,3})\\.armv6jel_vfp.rpm.*rpm to rollback list";

const std::string NewVersion = "^([A-Z?][a-z]{2})\\s(\\d{2})\\s(\\d{2}):(\\d{2}):(\\d{2}).*intouch-platform-base-(\\d)\\.(\\d)\\.(\\d)\\-(\\w+\\.\\d{2}|\\d{1,3}).*armv6jel_vfp.rpm\\s\\.\\.\\.";
