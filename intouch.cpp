...
std::ostream& operator<< (std::ostream &out, const Intouch &it) {
	...
	out << it.start_line_number << "(" << it.filename << "): " << it.start_time << " Boot Start" << std::endl;

	if(it.success == true) {
		out << it.end_line_number << "(" << it.filename  << "): " << it.end_time << " Boot Completed" << std::endl;
...
std::ostream& operator<< (std::ostream &out, const Services &service)
{
	...
	out << "\t\tStart: " << service.start_line_number << "(" << service.filename << ")" << std::endl;
	if(service.success == true) {
		out << "\t\tCompleted: "  << service.end_line_number << "(" << service.filename << ")" << std::endl;
